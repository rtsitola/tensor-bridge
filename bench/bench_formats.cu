...
#define RUN_FORMAT(NAME, TYPE, QUANT, DECODE)                                            \
    do {                                                                                 \
        ...
        TYPE *dA, *dB; half *A16, *B16; float *D;                                        \
        ...
        printf("  %-6s : %7.1f us/iter  %8.2f GFLOPS  err %6.3f%%\n",                   \
               NAME, ms/iters*1000, 2.0*M*N*K/(ms/1000.0/iters)/1e9,                    \
               100.0*sqrt(sse/snorm));                                                   \
        cudaFree(dA); cudaFree(dB); cudaFree(A16); cudaFree(B16); cudaFree(D);           \
...
#ifdef TARGET_SM75
    printf("Turing (sm_75): BF16, TF32, FP8, FP4 — all decoded -> FP16\n");
    RUN_FORMAT("BF16", uint16_t, tensor_bridge::fp32_to_bf16, decode_bf16_to_half);
    RUN_FORMAT("TF32", uint32_t, tensor_bridge::fp32_to_tf32, decode_tf32_to_half);
    RUN_FORMAT("FP8",  uint8_t,  tensor_bridge::fp32_to_fp8_e4m3, decode_fp8_to_half_fmt);
    RUN_FORMAT("FP4",  uint8_t,  tensor_bridge::fp32_to_fp4_e2m1, decode_fp4_to_half);
#endif
#ifdef TARGET_SM86
